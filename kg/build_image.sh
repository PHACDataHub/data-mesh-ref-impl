...
valid_images="keyphrase_extractor stanford_corenlp"
...
if [ $# -lt 1 ] || [ -z "$(echo ${valid_images} | grep $1)" ]; then
    ...
    echo "Example: ./build_image.sh keyphrase_extractor workflow.yaml main conf/keyphrase_extractor-requirements.txt conf/avro"
    exit
fi
...
if [ $IMAGE_NAME != "keyphrase_extractor" ]; then
