...
class Worker(object):
  ...
  def __init__(self, config):
    ...
    self.ner_tags = self.config['ner_tags'].split(',')

  def start(self):
    self.pipeline = stanza.Pipeline('en', processors='tokenize,ner')

  def process(self, msg_key, msg_val):
    if isinstance(msg_key, str):
      msg_key = { 'doc_id': msg_key }
    ...
    if text:
      ...
      if ne_dict:
        entity_types = list(set(ne_dict.keys()) & set(self.ner_tags))
        msg_val['ner'] = utils.to_json_str(ne_dict)
        if entity_types:
          match_ne_dict = {k: ne_dict[k] for k in entity_types}
          print(f"[FOUND] [{msg_key['doc_id']}] {entity_types} {match_ne_dict}")
        else:
          print(f"[ABSENT] [{msg_key['doc_id']}] {ne_dict.keys()} {ne_dict}")
