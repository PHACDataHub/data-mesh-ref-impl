...
class Worker(object):
  ...
  def __init__(self, config):
    ...
    self.length_limit = self.config['length_limit']

  def start(self):
    self.summarizer = pipeline("summarization", model=self.model, device=self.device)

  def process(self, msg_key, msg_val):
    found, results, text_list = False, dict(), []
    if 'headline' in msg_val and msg_val['headline']:
      text_list = [msg_val['headline']]
    if 'lead_para' in msg_val and msg_val['lead_para']:
      text_list.extend(msg_val['lead_para'])
    if 'tail_para' in msg_val and msg_val['tail_para']:
      text_list.extend(msg_val['tail_para'])

    texts = ['']
    for text in text_list:
      current_text = texts[-1]
      if self.length_limit > len(current_text) + len(text):
        texts[-1] = f"{current_text} {text}"
      else:
        texts.append(text)

    outputs = self.summarizer(texts)
    summary_texts = [e['summary_text'] for e in outputs]
    print(f"[SUMMARY] [{msg_key['doc_id']}] {' '.join(summary_texts)}")
    msg_val['summary_text'] = ' '.join(summary_texts)
