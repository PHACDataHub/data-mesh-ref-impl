...
class AvroClient(object):
    ...
    def __init__(self, config_section):
        ...
        self.schema_registry_client = SchemaRegistryClient({
            'url': f"http://{config_section['schema_registry_host']}:{config_section['schema_registry_port']}"
        })
...
class AvroConsumer(AvroClient):
    ...
    def __init__(self, config_section):
        ...
        self.consumer = Consumer({
            'bootstrap.servers': f"{config_section['broker_host']}:{config_section['broker_port']}",
...
class AvroProducer(AvroClient):
    ...
    def delivery_report(self, err, msg):
        if err is not None:
            print("Delivery failed for User record {}: {}".format(msg.key(), err), flush=True)
            return

        print('User record {} successfully produced to {} [{}] at offset {}'.format(
            msg.key(), msg.topic(), msg.partition(), msg.offset()), flush=True)
